...
define([
    ...
    AdminOrder.prototype = {
        ...
        /**
         * Bind event observer listeners to order type form fields
         *
         * @param container Div ID containing the order type form fields
         */
        orderTypeFieldsBind: function (container) {
            if ($(container)) {
                var fields = $(container).select('input', 'select');
                for (var i = 0; i < fields.length; i++) {
                    if (fields[i].id === 'order-type-dropdown') {
                        // Select box form field
                        fields[i].observe('change', this.orderTypeDropdownChange.bind(this));
                    } else {
                        // Input form field
                        fields[i].observe('change', this.referenceOrderInputChange.bind(this));
                        jQuery(fields[i].parentElement.parentElement).hide();
                    }
                }
                // Display 'order-additional_area' div, because it is hidden by default
                $('order-additional_area').show();
            }
        },

        /**
         * Toggle visibility of reference order input field based on value selected from order type dropdown
         *
         * @param event The event dispatched on click
         */
        orderTypeDropdownChange: function (event) {
            if (event.target.selectedOptions[0].value === "2" || event.target.selectedOptions[0].value === "3") {
                // Show reference-order-input field
                jQuery('.field-reference-order-input').show();
            } else {
                jQuery('.field-reference-order-input').hide();
            }
        },

        /**
         * Serialise order type fields and submit ajax request to save the values
         *
         * @param event The event dispatched on click
         */
        referenceOrderInputChange: function (event) {
            // Prevent submissions of empty reference order input field
            if (event.currentTarget.value !== '') {
                this.saveData(this.serializeData('order-additional_area'));
            }
        },
