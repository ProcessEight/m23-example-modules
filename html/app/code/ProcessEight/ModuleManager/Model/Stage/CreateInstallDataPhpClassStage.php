...
class CreateInstallDataPhpClassStage extends BaseStage
{
    ...
    public function configureStage(array $payload) : array
    {
        ...
        $payload['config'][$this->id]['options'][ConfigKey::SETUP_VERSION] = [
            'name'                    => ConfigKey::SETUP_VERSION,
            'shortcut'                => null,
            'mode'                    => \Symfony\Component\Console\Input\InputOption::VALUE_REQUIRED,
            'description'             => 'Setup version no, in semantic versioning format (e.g. 1.0.0)',
            'question'                => '<question>Setup version no, in semantic versioning format (e.g. 1.0.0) [1.0.0]: </question> ',
            'question_default_answer' => '1.0.0',
        ];
    ...
    public function getTemplateVariables(string $stageId, array $payload) : array
    {
        return [
            ...
            '{{YEAR}}'                  => date('Y'),
            '{{SETUP_VERSION}}'         => $payload['config'][$stageId]['values'][ConfigKey::SETUP_VERSION],
        ];
    }
}
